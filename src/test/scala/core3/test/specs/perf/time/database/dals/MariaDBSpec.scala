...
trait MariaDBSpec extends PerformanceTimeSpec {
  ...
  performance of "MariaDB" in {
    measure method "createObject" in {
      using(containerCount)
        ...
        .beforeTests {
          MariaDBSpec.db.clearDatabaseStructure("TransactionLog").ignoreFailure.await
          MariaDBSpec.db.buildDatabaseStructure("TransactionLog").await
        }
        .in {
          count => MariaDBSpec.createContainers("TransactionLog", count).await
        }
    }
    ...
    measure method "queryDatabase" in {
      using(containerCount)
        ...
        .beforeTests {
          MariaDBSpec.db.clearDatabaseStructure("TransactionLog").ignoreFailure.await
          MariaDBSpec.db.buildDatabaseStructure("TransactionLog").await
          MariaDBSpec.createContainers("TransactionLog", maxContainers).await
        }
        .in {
          count =>
            0 until count foreach {
              _ =>
                val result = MariaDBSpec.db.queryDatabase("TransactionLog").await
...
object MariaDBSpec {
  val db: DatabaseAbstractionLayer = core3.test.fixtures.Database.createMariaDBInstance()
