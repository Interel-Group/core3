...
trait MemoryOnlyDBSpec extends PerformanceTimeSpec {
  val maxContainers = 1000
  val containerCount: Gen[Int] = Gen.range("containerCount")(from = 0, upto = maxContainers, hop = 100)

  performance of "MemoryOnlyDB" in {
    measure method "createObject" in {
      using(containerCount)
        ...
        .beforeTests {
          MemoryOnlyDBSpec.db.clearDatabaseStructure("TransactionLog").ignoreFailure.await
          MemoryOnlyDBSpec.db.buildDatabaseStructure("TransactionLog").await
        }
        .in {
          count =>
            MemoryOnlyDBSpec.createContainers("TransactionLog", count).await
        }
    }
    ...
    measure method "queryDatabase" in {
      using(containerCount)
        ...
        .beforeTests {
          MemoryOnlyDBSpec.db.clearDatabaseStructure("TransactionLog").ignoreFailure.await
          MemoryOnlyDBSpec.db.buildDatabaseStructure("TransactionLog").await
          MemoryOnlyDBSpec.createContainers("TransactionLog", maxContainers).await
        }
        .in {
          count =>
...
object MemoryOnlyDBSpec {
  val db: DatabaseAbstractionLayer = core3.test.fixtures.Database.createMemoryOnlyDBInstance()
