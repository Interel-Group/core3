...
object QueryableContainer extends JsonContainerCompanion with SlickContainerCompanion {
  ...
  private class TableDef(tag: Tag)
    extends Table[QueryableContainer](tag, "core_queryable_container") {
  ...
  private val compiledGetByCurrency = Compiled((currency: Rep[Currency]) => query.filter(_.currency === currency))
  private val compiledGetAfterTimestamp = Compiled((timestamp: Rep[Timestamp]) => query.filter(_.timestamp > timestamp))
  private val compiledGetBeforeTimestamp = Compiled((timestamp: Rep[Timestamp]) => query.filter(_.timestamp < timestamp))
  private val compiledGetByDate = Compiled((date: Rep[Date]) => query.filter(_.date === date))
  private val compiledGetAfterTime = Compiled((time: Rep[Time]) => query.filter(_.time > time))
  private val compiledGetBeforeTime = Compiled((time: Rep[Time]) => query.filter(_.time < time))
  private val compiledGetByAnyEnum = Compiled((enum: Rep[TestEnum]) => query.filter(c => c.enum === enum || c.enumOpt === enum))
  private val compiledGetByOptionalCurrency = Compiled((currency: Rep[Option[Currency]]) => query.filter(_.currencyOpt === currency))
  private val compiledGetAfterOptionalTimestamp = Compiled((timestamp: Rep[Option[Timestamp]]) => query.filter(_.timestampOpt > timestamp))
  private val compiledGetBeforeOptionalTimestamp = Compiled((timestamp: Rep[Option[Timestamp]]) => query.filter(_.timestampOpt < timestamp))
