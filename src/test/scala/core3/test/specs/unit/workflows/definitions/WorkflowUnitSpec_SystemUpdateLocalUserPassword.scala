...
class WorkflowUnitSpec_SystemUpdateLocalUserPassword extends AsyncUnitSpec {
  ...
  private val workflows = Vector(SystemCreateLocalUser, SystemUpdateLocalUserPassword)
  ...
  "A SystemUpdateLocalUserPassword workflow" should "successfully update users" in {
    fixture =>
      ...
      val updatedPassword = "test!"
      val updatedSalt = "qwerty!@#$"
      ...
      for {
        ...
        updatedUsers.head.hashedPassword should be(updatedPassword)
        updatedUsers.head.passwordSalt should be(updatedSalt)
        updatedUsers.head.permissions should be(testPermissions)
        updatedUsers.head.userType should be(testUserType)
        updatedUsers.head.metadata should be(testMetadata)
      }
  }
}
