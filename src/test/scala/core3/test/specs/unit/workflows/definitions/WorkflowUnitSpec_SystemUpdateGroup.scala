...
class WorkflowUnitSpec_SystemUpdateGroup extends AsyncUnitSpec {
  ...
  private val workflows = Vector(SystemCreateGroup, SystemUpdateGroup)
  ...
  "A SystemUpdateGroup workflow" should "successfully update groups" in {
    fixture =>
      ...
      val updatedTestName = "New Name"
      val updatedTestItems = Vector(core3.database.getNewObjectID, core3.database.getNewObjectID)
      ...
      for {
        ...
        updatedGroups should have size 1
        updatedGroups.head.name should be(updatedTestName)
        updatedGroups.head.shortName should be(testShortName)
        updatedGroups.head.itemsType should be(testItemsType)
        updatedGroups.head.items should be(updatedTestItems)
      }
  }
}
