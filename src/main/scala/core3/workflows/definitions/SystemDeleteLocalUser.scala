...
object SystemDeleteLocalUser extends WorkflowBase {

  case class SystemDeleteLocalUserParameters(userUUID: ObjectID, revision: RevisionID, revisionNumber: RevisionSequenceNumber) extends WorkflowParameters {
  ...
  case class SystemDeleteLocalUserInputData(user: core.LocalUser) extends InputData {
    override def asJson: JsValue = Json.obj(
  ...
  override def parseParameters(rawParams: JsValue)(implicit ec: ExecutionContext): Future[WorkflowParameters] = {
    Future {
      SystemDeleteLocalUserParameters(
  ...
  override def loadData(params: WorkflowParameters, queryHandlers: DataQueryHandlers)(implicit ec: ExecutionContext): Future[InputData] = {
    params match {
      case actualParams: SystemDeleteLocalUserParameters =>
        for {
          ...
          SystemDeleteLocalUserInputData(user)
        }
    }
  }
  ...
  override def executeAction(requestID: RequestID, user: UserTokenBase, params: WorkflowParameters, data: InputData)(implicit ec: ExecutionContext): Future[(WorkflowResult, OutputData)] = {
    data match {
      case actualData: SystemDeleteLocalUserInputData =>
        Future.successful((WorkflowResult(wasSuccessful = true, requestID), OutputData(delete = Vector(actualData.user))))
    }
  }
}
